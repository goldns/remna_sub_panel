...
- Кнопка «Продлить» в шапке панели с настраиваемым URL-шаблоном (`{shortUuid}`, `{uuid}`, `{B64:USERNAME}`)
...
В шапке браузерной панели можно показать до двух кнопок оплаты:

| Параметр | Кнопка | Описание |
|---|---|---|
| `payment_url` | **Продлить** | Ссылка на сайт оплаты |
| `payment_url_tg` | **Продлить в TG** | Ссылка на Telegram-бота оплаты |
